...
function Dashboard() {
  ...
  const filteredDeals = useMemo(() => {
    ...
    switch (dealFilter) {
      case 'open':
        deals = deals.filter(d => !['closed-won', 'closed-lost'].includes(d.stage))
        break
      case 'at-risk':
        deals = deals.filter(d => d.probability < 40 && !['closed-won', 'closed-lost'].includes(d.stage))
        break
      case 'won':
  ...
  const tableSummary = useMemo(() => getTableSummary(filteredDeals), [filteredDeals])
  ...
  const dealCounts = useMemo(() => ({
    ...
    open: mockDeals.filter(d => !['closed-won', 'closed-lost'].includes(d.stage)).length,
    'at-risk': mockDeals.filter(d => d.probability < 40 && !['closed-won', 'closed-lost'].includes(d.stage)).length,
  ...
  const activityData = useMemo(() => getDealActivities(), [])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownButton
              label={DATE_FILTER_OPTIONS.find(o => o.value === dateFilter)?.label || 'This Quarter'}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2">
            <Card className="p-6 h-full">
              <Heading as="h3" size="lg" className="mb-4">Pipeline Distribution</Heading>
        ...
        <Tabs defaultValue="all" value={dealFilter} onChange={handleFilterChange}>
          <div className="flex items-center justify-between mb-4">
            <TabList>
              <TabTrigger value="all">
                All Deals
                <Badge variant="default" size="sm" className="ml-2">{dealCounts.all}</Badge>
              </TabTrigger>
              <TabTrigger value="open">
                Open
                <Badge variant="info" size="sm" className="ml-2">{dealCounts.open}</Badge>
              </TabTrigger>
              <TabTrigger value="at-risk">
                At Risk
                <Badge variant="error" size="sm" className="ml-2">{dealCounts['at-risk']}</Badge>
              </TabTrigger>
              <TabTrigger value="won">
                Won
                <Badge variant="success" size="sm" className="ml-2">{dealCounts.won}</Badge>
              </TabTrigger>
              <TabTrigger value="lost">
                Lost
                <Badge variant="default" size="sm" className="ml-2">{dealCounts.lost}</Badge>
              </TabTrigger>
            </TabList>
          </div>
        </Tabs>
        ...
        <Tabs defaultValue="all" value={dealFilter} onChange={handleFilterChange} className="mb-8">
          <div>
            {/* Deals Table */}
                  ...
                  <Table>
                    <TableHeader sticky>
                      <TableRow>
                        ...
                        <TableHead
                          sortable
                          sortDirection={sortConfig?.key === 'probability' ? sortConfig.direction : null}
                      ...
                      <TableBody>
                        {paginatedDeals.map((deal) => (
                          ...
                          >
                            <TableCell>
                              <div>
                                <Text weight="medium" className="text-gray-900">{deal.name}</Text>
                                <Text size="sm" color="muted">{deal.company}</Text>
                              </div>
                            </TableCell>
                            <TableCell>
                              <Text weight="semibold">{formatCurrency(deal.value)}</Text>
                            </TableCell>
                            <TableCell>
                              <StageIndicator currentStage={deal.stage} variant="compact" showLabels={false} />
                            </TableCell>
                            <TableCell>
                              <Badge variant={getStageBadgeVariant(deal.stage)} size="sm">
                            ...
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <ProgressBar
                                  ...
                                  className="w-16"
                                />
                                <Text size="sm" color="muted">{deal.probability}%</Text>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <Avatar name={deal.owner} size="sm" />
                                <Text size="sm" color="muted">{deal.owner}</Text>
                              </div>
                            </TableCell>
                            <TableCell onClick={(e) => e.stopPropagation()}>
                              <Dropdown
                                ...
                                onChange={(action) => {
                                  ...
                                  addToast({
                                    type: action === 'archive' ? 'warning' : 'success',
                                    title: action === 'view' ? 'Opening deal...' : `Deal action: ${action}`,
                    ...
                    <TableFooter>
                      <TableRow>
                        <TableCell>
                          <Text weight="semibold">Total: {filteredDeals.length} deals</Text>
                        </TableCell>
                        <TableCell>
                          <Text weight="semibold">{formatCurrency(tableSummary.totalValue)}</Text>
                        </TableCell>
                        <TableCell>{' '}</TableCell>
                        <TableCell>{' '}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Text size="sm" color="muted">Avg:</Text>
                            <TrendIndicator value={tableSummary.avgProbability} format="percent" showIcon={false} />
                          </div>
                        </TableCell>
                        <TableCell>{' '}</TableCell>
                        <TableCell>{' '}</TableCell>
            ...
            <Card className="p-6 mt-8">
              <div className="flex items-center justify-between mb-4">
                <Heading as="h3" size="lg">Real-time Alerts</Heading>
                <a
                  href="#"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <Heading as="h3" size="lg">Pipeline Summary</Heading>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between">
                <Text size="sm" weight="medium" color="muted">Total Pipeline</Text>
                <Text size="lg" weight="bold">{totalPipeline}</Text>
              </div>
            </div>
          </Card>
          ...
          {/* Team Section */}
          <Card className="p-6">
            <Heading as="h3" size="lg" className="mb-4">Active Team</Heading>
