...
export function DataList({
  ...
  if (variant === 'horizontal') {
    return (
      <dl className={`grid gap-4 ${columnClasses[columns]} ${className}`} {...props}>
        {items.map((item, index) => (
          <div key={index} className="flex items-start justify-between gap-4 py-2">
            <dt className="text-sm text-gray-500 flex-shrink-0">{item.label}</dt>
  ...
  if (variant === 'striped') {
    return (
      <dl className={`rounded-lg border border-gray-200 overflow-hidden ${className}`} {...props}>
        {items.map((item, index) => (
          <div
            key={index}
            ...
            {item.change && (
              <span
                className={`text-xs font-medium ${
                  item.change.direction === 'up' ? 'text-[#5dcb8a]' : 'text-red-500'
                }`}
              >
                {item.change.direction === 'up' ? '+' : '-'}{item.change.value}%
              </span>
            )}
          </div>
        ))}
