...
export function DealDetailsModal({
  ...
  const stageForIndicator = deal.stage === 'closed-lost' ? 'closed-won' : deal.stage
  ...
  const activityItems: ActivityItem[] = deal.recentActivity?.map((activity, index) => ({
    id: `activity-${index}`,
        ...
        <div className="bg-gradient-to-r from-[#f5faf7] via-white to-white border-b border-gray-200 px-8 py-6 relative">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="mb-4">
              ...
              <Text size="lg" color="muted" className="mb-3">{deal.company}</Text>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <ul className="space-y-3">
                    {deal.nextSteps.map((step, index) => (
                      <li key={index} className="flex items-start gap-3">
                        ...
                        <Text color="muted" className="flex-1">{step}</Text>
                      </li>
                    ))}
                  </ul>
                </Card>
            ...
            <div className="space-y-6">
              <Card className="p-6">
                ...
                <div className="space-y-4">
                  <div>
                    <Text size="sm" color="muted" className="mb-1">Last Activity</Text>
                    <Text weight="medium">{deal.lastActivity}</Text>
                  </div>
                  <Divider variant="dashed" />
                  <div>
                    <Text size="sm" color="muted" className="mb-1">Deal ID</Text>
                    <Text weight="medium" className="font-mono">{deal.id}</Text>
                  </div>
                  <Divider variant="dashed" />
                  <div>
                    <Text size="sm" color="muted" className="mb-1">Stage Progress</Text>
                    <div className="flex items-center gap-2 mt-2">
                      <ProgressBar
                        value={deal.stage === 'closed-won' ? 100 : deal.stage === 'closed-lost' ? 0 : ((currentStageIndex + 1) / PIPELINE_STAGES.length) * 100}
                        variant={deal.stage === 'closed-won' ? 'success' : deal.stage === 'closed-lost' ? 'error' : 'default'}
                      />
                      <Text size="sm" color="muted">
                        {deal.stage === 'closed-won' ? '100%' : deal.stage === 'closed-lost' ? '0%' : `${Math.round(((currentStageIndex + 1) / PIPELINE_STAGES.length) * 100)}%`}
                      </Text>
                    </div>
                  </div>
                </div>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Text size="sm" color="muted">Probability</Text>
                    <Badge
                      variant={
                      ...
                      size="sm"
                    >
                      {deal.probability >= 70 ? 'High' :
                       deal.probability >= 40 ? 'Medium' : 'Low'}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <Text size="sm" color="muted">Engagement</Text>
                    <Badge variant="success" size="sm">Active</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <Text size="sm" color="muted">Data Quality</Text>
